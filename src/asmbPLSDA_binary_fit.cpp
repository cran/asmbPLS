...
List asmbPLSDA_binary_fit(arma::mat E_matrix, 
  ...
  String outcome_type = "binary"; // Set outcome_type to binary
  
  arma::rowvec X_col_mean; // Column mean for X
  arma::rowvec X_col_sd; // Column sd for X
  double Y_col_mean = 0.5; // Column mean for Y
  
  // Center and scale
  if(center[0]) {
    // Extract index for samples from different groups
    arma::mat E_matrix_1 = E_matrix.rows(find(F_matrix == 0));
    arma::mat E_matrix_2 = E_matrix.rows(find(F_matrix == 1));
    
    // Center for X
    arma::rowvec col_mean_1 = mean(E_matrix_1, 0);
    arma::rowvec col_mean_2 = mean(E_matrix_2, 0);
    X_col_mean = (col_mean_1 + col_mean_2)/2; // Obtain weighted column mean
  ...
  for (int i = 0; i < PLS_term; ++i) {
    ...
    while (accu(abs(t_diff)) > 0.000001 && n_iter <= maxiter) {
      ...
      for (int j = 0; j < B; ++j) {
        ...
        w_temp = as<arma::colvec>(weight_sparse(w_temp, l_temp));
